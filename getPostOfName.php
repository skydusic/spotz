<?php
    ...
    $total = $total_record + $total_record;
    ...
    for ($i=0; $i < $total_record2; $i++)
    {
      ...
      echo "{\"listname\":\"clubtable\",\"idx\":\"$row2[idx]\",\"postidx\":\"$row2[postidx]\",\"contents\":\"$row2[contents]\",\"username\":\"$row2[username]\",\"created\":\"$row2[created]\",\"image\":\"$row2[image]\",\"hit\":\"$row2[hit]\",\"postidx\":\"$rowex[postidx]\",\"text1\":\"$row2[corperation]\",\"text2\":\"$row2[sports]\",\"text3\":\"$row2[location]\",\"text4\":\"$row2[phone]\",\"text5\":\"$row2[etc]\"}";
